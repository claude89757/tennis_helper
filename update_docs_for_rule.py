...
if __name__ == '__main__':
    ...
    for rule in all_rule_list:
        ...
        first_line_infos[f"G{index}"] = str(rule['duration'])
        first_line_infos[f"H{index}"] = status_name_infos.get(str(rule['status']), str(rule['status']))
        first_line_infos[f"I{index}"] = str(rule['jrtzcs'])
        first_line_infos[f"J{index}"] = str(rule['zjtzcs'])
        first_line_infos[f"K{index}"] = str(rule['sjwh'])
        first_line_infos[f"L{index}"] = user_level_infos.get(str(rule['user_level']), str(rule['user_level']))
