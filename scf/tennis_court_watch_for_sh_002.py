...
def generate_auth_params(data, secret_key):
    """
    生成鉴权参数，包括更新时间戳、随机字符串和签名。
    """
    # 更新时间戳和随机字符串
    data['client_time'] = str(int(time.time() * 1000))
    data['noncestr'] = hashlib.md5(data['client_time'].encode()).hexdigest()[:16]

    # 生成签名
    sorted_params = sorted(data.items())
    sign_string = ''.join(f"{key}={value}" for key, value in sorted_params if key != 'api_sign')
    sign_string += secret_key
    data['api_sign'] = hashlib.sha256(sign_string.encode()).hexdigest()
...
def get_free_tennis_court_data(order_date: str, proxy_list: list = None, ok_proxy_list: list = None):
    ...
    data = {
        ...
        "client_time": "",
        "sign_key": "1721641166072531800",
        "device_id": "17213697251829934155",
        "app_version": "3.0.9",
        "latitude": "22.53092098",
        "longitude": "113.94631577",
        "noncestr": "",
        "access_token": "gJLnsXwq8oa6IIYx4rnWH1ddoqq7VB9f",
        "login_token": "93818ea0335dbe2e96f1a555e6993bc3",
        "city_id": "321",
        "city_name": "上海",
        "ver": "2.9",
        "venues_id": "22377",
    ...
    # 生成鉴权参数
    print(data)
    secret_key = "1721641166072531800"  # 假设sign_key就是secret_key
    data = generate_auth_params(data, secret_key)
    print(data)

    response = None
    if proxy_list or ok_proxy_list:
        all_proxy_list = proxy_list + (ok_proxy_list or [])
        for proxy in all_proxy_list:
            try:
                ...
                print(f"Exception with proxy {proxy}: {e}")
    else:
    ...
    if response and response.status_code == 200:
        res = response.json()
        print(res)
        return res, success_proxy_list
    else:
        raise Exception("Unknown error or no successful response")

# Example usage
try:
    result, proxies_used = get_free_tennis_court_data("2024-07-27")
    print("Result:", result)
